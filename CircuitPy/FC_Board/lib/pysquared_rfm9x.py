...
_RH_RF95_REG_FDEV_UPPER = const(0x04)
_RH_RF95_REG_FDEV_LOWER = const(0x05)
...
class RFM9x:
    ...
    if self.long_range_mode:
        # cache LoRa params
        cache = [self.spreading_factor, self.signal_bandwidth, self.coding_rate, self.preamble_length, self.enable_crc]
        self.operation_mode = SLEEP_MODE
        time.sleep(0.01)
    ...
    self._write_u8(0x03, 0x05)  # BitRate(7:0)
    self._write_u8(0x05, 39)  # Freq deviation 2380 Hz
    self.idle()
